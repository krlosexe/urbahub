    ...
    */
    function admin(tbody, table){
        $(tbody).on("click", "span.admin", function(){
            ...
            $("#monto_cobranza_registrar").val(number_format(data.mensualidad, 2));
            $("#saldo_cobranza_registrar").val(number_format(data.mensualidad, 2));
    ...
    function savepago(form, controlador, cuadro){
        $(form).submit(function(e){
            ...
            if(inNum($("#monto_pago").val())< inNum($("#monto").val())){
                mensajes('danger', "<span> El monto de pago debe ser igual al monto del recibo</span>");     
                return false;
            }
            var comprobantes = [];
