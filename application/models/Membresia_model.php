    ...
    */
    public function listado_recargos_saldos($id_mem,$fecha,$numero_renovacion,$actual){
        ...
        if($fecha == ""){
            ...
            #Si el filtro fecha no esta en blanco visualizo toda la info de membresia hasta la fecha seleccionada es por ello que es menor igual lte
            $fecha_mongo = new MongoDB\BSON\UTCDatetime(strtotime($fecha)*1000);
            $res_membresia = $this->mongo_db->order_by(array('_id' => 'DESC'))->where(array('eliminado'=>false,'_id'=>$id))->where_lte('fecha_inicio', $fecha_mongo)->get("membresia");
        }*/
        if(count($res_membresia)>0){
            ...
            foreach ($servicios_arreglo as $clave_servicios => $valor_servicios) {
                ...
                $titulo = $this->consultar_titulo_servicios($valor_servicios->servicios);
